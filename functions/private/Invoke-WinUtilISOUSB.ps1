...
function Invoke-WinUtilISOWriteUSB {
    ...
    if (-not $targetDisk) {
        Show-WinUtilMessage -Message "请从下拉列表中选择 USB 驱动器。" -Title "未选择驱动器" -Button "OK" -Icon "Warning" | Out-Null
        return
    }
    ...
    $confirm = Show-WinUtilMessage -Message "磁盘 $diskNum（$($targetDisk.FriendlyName)，$sizeGB GB）上的所有数据将被永久删除。`n`n确定要继续吗？" -Title "确认擦除 USB" -Button "YesNo" -Icon "Warning"
    if ($confirm -ne "Yes") {
    ...
    Start-WinUtilJob -Name "USB write" -Description "正在写入 U 盘" -Parameters @{
        ...
        try {
            ...
            Show-WinUtilMessage -Message "USB 写入失败：`n`n$_" -Title "USB 写入出错" -Button "OK" -Icon "错误" | Out-Null
            throw
        } finally {
