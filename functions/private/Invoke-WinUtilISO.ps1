...
function Write-WinUtilISOLog {
    ...
    Invoke-WPFUIThread -Async -Parameters @{
        ...
        if ($box.Text -eq "就绪。请先选择 Windows 11 ISO 文件。") {
            $box.Text = $LogLine
        } else {
            $box.Text += "`n$LogLine"
        }
...
function Invoke-WinUtilISOMountAndVerify {
    ...
    if ([string]::IsNullOrWhiteSpace($isoPath) -or $isoPath -eq "尚未选择 ISO...") {
        Show-WinUtilMessage -Message "请先选择 ISO 文件。" -Title "未选择 ISO" -Button "OK" -Icon "Warning" | Out-Null
        return
    }
    ...
    Start-WinUtilJob -Name "ISO mount" -Description "正在挂载 ISO" -Parameters @{
        ...
        try {
            ...
            if (-not (Test-Path $wimPath) -and -not (Test-Path $esdPath)) {
                ...
                Show-WinUtilMessage -Message "这似乎不是有效的 Windows ISO。`n`n未找到 install.wim / install.esd。" -Title "无效的 ISO" -Button "OK" -Icon "错误" | Out-Null
            ...
            if (-not ($imageInfo | Where-Object { $_.ImageName -match "Windows 11" })) {
                ...
                Show-WinUtilMessage -Message "此 ISO 中未找到 Windows 11 版本。`n`n仅支持官方的 Windows 11 ISO。" -Title "不是 Windows 11 ISO" -Button "OK" -Icon "错误" | Out-Null
...
function Invoke-WinUtilISOModify {
    ...
    if (-not $isoPath) {
        Show-WinUtilMessage -Message "未找到已验证的 ISO。请先完成步骤 1 和 2。" -Title "未就绪" -Button "OK" -Icon "Warning" | Out-Null
        return
    }
    ...
    Start-WinUtilJob -Name "ISO modify" -Description "正在修改 ISO" -Parameters @{
        ...
        try {
            ...
            Show-WinUtilMessage -Message "修改 install.wim 时出错：`n`n$_" -Title "修改出错" -Button "OK" -Icon "错误" | Out-Null
...
function Invoke-WinUtilISOCheckExistingWork {
    ...
    Show-WinUtilMessage -Message "找到之前的 WinUtil ISO 工作目录：`n`n$($existingWorkDir.FullName)`n`n（上次修改时间：$modified）`n`n步骤 4（输出选项）已恢复，以便您保存已修改的映像。`n`n如果要重新开始，请点击步骤 4 中的「清理并重置」。" -Title "发现现有工作" -Button "OK" -Icon "Info" | Out-Null
}
...
function Invoke-WinUtilISOCleanAndReset {
    ...
    if ($workDir -and (Test-Path $workDir)) {
        $confirm = Show-WinUtilMessage -Message "这将删除临时工作目录：`n`n$workDir`n`n并将界面重置回开始状态。`n`n是否继续？" -Title "清理并重置" -Button "YesNo" -Icon "Warning"
    ...
    Start-WinUtilJob -Name "ISO cleanup" -Description "正在清理" -Parameters @{
        ...
        try {
            ...
            Invoke-WPFUIThread -ScriptBlock {
                ...
                $sync["WPFWin11ISOStatusLog"].Text               = "就绪。请先选择 Windows 11 ISO 文件。"
            }
...
function Invoke-WinUtilISOExport {
    ...
    if (-not $contentsDir -or -not (Test-Path $contentsDir)) {
        Show-WinUtilMessage -Message "未找到修改后的 ISO 内容。请先完成步骤 1-3。" -Title "未就绪" -Button "OK" -Icon "Warning" | Out-Null
        return
    }
    ...
    Start-WinUtilJob -Name "ISO export" -Description "正在构建 ISO" -Parameters @{
        ...
        try {
            ...
            if (-not $oscdimg) {
                Show-WinUtilMessage -Message "无法找到或自动安装 oscdimg.exe。`n`n请手动安装：`n  winget install -e --id Microsoft.OSCDIMG`n`n或从以下链接安装 Windows ADK：`nhttps://learn.microsoft.com/windows-hardware/get-started/adk-install" -Title "未找到 oscdimg" -Button "OK" -Icon "Warning" | Out-Null
            ...
            Show-WinUtilMessage -Message "ISO 导出成功！`n`n$outputISO" -Title "导出完成" -Button "OK" -Icon "Info" | Out-Null
        } catch {
            Write-WinUtilISOLog -Level "ERROR" -Message "ISO export failed: $_"
            $_.Exception.Data["WinUtilErrorReported"] = $true
            Show-WinUtilMessage -Message "ISO 导出失败：`n`n$_" -Title "错误" -Button "OK" -Icon "错误" | Out-Null
            throw
        } finally {
